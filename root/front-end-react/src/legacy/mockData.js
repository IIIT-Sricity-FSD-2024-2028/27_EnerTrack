...
function repairDemoPasswords() {
  ...
  users.forEach((user) => {
    if (
      user.password === "hashed_password_mock" &&
      demoPasswords[user.user_id]
    ) {
...
function repairDemoEmails() {
  ...
  users.forEach((user) => {
    if (
      demoEmails[user.user_id] &&
      /@enertrack\.edu$/i.test(user.email || "")
    ) {
...
function migrateRegisteredUsers() {
  ...
  try {
    ...
    registeredUsers.forEach((user) => {
      ...
      if (
        !email ||
        users.some((existing) => normalizeEmail(existing.email) === email)
      )
        return;
  ...
  authenticate(username, password) {
    ...
    const found = users.find((user) => {
      ...
      const passwordMatches =
        ...
        user.password === mockHashPassword(password);
...
export function isEmailTaken(email) {
  ...
  return userActions
    .getAllUsers()
    .some((user) => normalizeEmail(user.email) === normalizedEmail);
}

export function isPhoneTaken(phone) {
  return userActions
    .getAllUsers()
    .some((user) => user.phone && user.phone === phone);
}
