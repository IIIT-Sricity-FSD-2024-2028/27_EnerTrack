...
export function renderBellIcon(containerId, email) {
  ...
  const badgeHTML =
    count > 0
      ? `<span style="position:absolute;top:-4px;right:-4px;background:#dc2626;color:white;font-size:9px;font-weight:700;min-width:16px;height:16px;border-radius:8px;display:flex;align-items:center;justify-content:center;padding:0 4px;">${count > 9 ? "9+" : count}</span>`
      : "";
  ...
  document.getElementById("notif-bell").addEventListener("click", (e) => {
    ...
    dropdown.style.cssText =
      "position:absolute;top:32px;right:0;width:320px;max-height:360px;overflow-y:auto;background:white;border:1px solid #e5e7eb;border-radius:10px;box-shadow:0 8px 24px rgba(0,0,0,0.12);z-index:9999;";
...
export function notifyOnStateChange(report, action, actorName) {
  ...
  const soEmails = users
    .filter((u) => _normalizeRole(u.role) === "sustainability_officer")
    .map((u) => u.email);
  const faEmails = users
    .filter((u) => _normalizeRole(u.role) === "finance_analyst")
    .map((u) => u.email);
  ...
  switch (action) {
    case "submitted":
      soEmails.forEach((e) =>
        pushNotification(e, {
          reportId: report.id,
          message: `${actorName} submitted a new ${report.type} wastage report (${reportLabel}).`,
        }),
      );
      break;
    case "validated":
      pushNotification(reporterEmail, {
        reportId: report.id,
        message: `Your ${reportLabel} has been validated by the Sustainability Officer.`,
      });
      break;
    case "dismissed":
      pushNotification(reporterEmail, {
        reportId: report.id,
        message: `Your ${reportLabel} was dismissed: ${report.dismissReason || "See details"}. It has been moved to My Archives.`,
      });
      soEmails.forEach((e) =>
        pushNotification(e, {
          reportId: report.id,
          message: `${actorName} dismissed ${reportLabel}. Reason: ${report.dismissReason || "Not specified"}.`,
        }),
      );
      faEmails.forEach((e) =>
        pushNotification(e, {
          reportId: report.id,
          message: `${reportLabel} was dismissed by Sustainability Officer (${report.dismissReason || "No reason"}).`,
        }),
      );
      break;
    case "forwarded":
      faEmails.forEach((e) =>
        pushNotification(e, {
          reportId: report.id,
          message: `${reportLabel} has been forwarded to you for cost impact assessment.`,
        }),
      );
      pushNotification(reporterEmail, {
        reportId: report.id,
        message: `Your ${reportLabel} has been forwarded to Finance for cost assessment.`,
      });
      break;
    case "cost_added":
      soEmails.forEach((e) =>
        pushNotification(e, {
          reportId: report.id,
          message: `Finance has returned ${reportLabel} with cost impact figures.`,
        }),
      );
      pushNotification(reporterEmail, {
        reportId: report.id,
        message: `Financial assessment completed for your ${reportLabel}.`,
      });
      break;
    case "returned_to_so":
      soEmails.forEach((e) =>
        pushNotification(e, {
          reportId: report.id,
          message: `${reportLabel} has been returned by Finance with cost figures.`,
        }),
      );
      break;
    case "target_set":
      pushNotification(reporterEmail, {
        reportId: report.id,
        message: `A new sustainability target has been set for your ${reportLabel}.`,
      });
      break;
    case "delivered":
      pushNotification(reporterEmail, {
        reportId: report.id,
        message: `Final report delivered for ${reportLabel}. Check your wastage logs for the full breakdown.`,
      });
      break;
  }
}
...
function _normalizeRole(role) {
  ...
  if (r === "finance analyst" || r === "financial analyst")
