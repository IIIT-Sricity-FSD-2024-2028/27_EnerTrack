...
import {
  ...
  getMetricCategory,
  getBaselineRange,
} from "../shared/sensorSimulator.js";
import {
  renderBellIcon,
  notifyOnStateChange,
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function saveFormState() {
    ...
    document
      .querySelectorAll(
        '#wastageReportForm input:not([type="hidden"]), #wastageReportForm textarea',
      )
      .forEach((el) => {
  ...
  function updateDynamicFields(selectedValue) {
    ...
    document
      .querySelectorAll("#dynamicFields input, #dynamicFields textarea")
      .forEach((el) => (el.required = false));
    if (selectedValue) {
      const activeFields = document.querySelectorAll(
        `#dynamicFields .field-${selectedValue} input, #dynamicFields .field-${selectedValue} textarea`,
      );
  ...
  document
    ...
    .addEventListener("submit", async (e) => {
      ...
      const formElements = document.querySelectorAll(
        "#wastageReportForm input, #wastageReportForm textarea, #wastageReportForm select",
      );
      ...
      if (issueType === "Energy") {
        specificData.building = document
          .getElementById("energyBuilding")
          .value.trim();
        specificData.area = document.getElementById("energyArea").value.trim();
        specificData.observation = document
          .getElementById("energyObservation")
          .value.trim();
        const reported = Number(
          document.getElementById("energyEstimatedValue").value,
        );
        ...
        specificData.location = document
          .getElementById("waterLocation")
          .value.trim();
        specificData.nature = document
          .getElementById("waterNature")
          .value.trim();
        const reported = Number(
          document.getElementById("waterEstimatedValue").value,
        );
        if (Number.isFinite(reported) && reported > 0) {
          specificData.reportedValue = reported;
        ...
        specificData.source = document
          .getElementById("emissionsSource")
          .value.trim();
        specificData.location = document
          .getElementById("emissionsLocation")
          .value.trim();
        specificData.observation = document
          .getElementById("emissionsObservation")
          .value.trim();
        const reported = Number(
          document.getElementById("emissionsEstimatedValue").value,
        );
        if (Number.isFinite(reported) && reported > 0) {
          specificData.reportedValue = reported;
        ...
        specificData.cafeteria = document
          .getElementById("foodCafeteria")
          .value.trim();
        specificData.typeOfWastage = document
          .getElementById("foodType")
          .value.trim();
        specificData.estimatedAmount = document
      ...
      if (!baseline || !Number.isFinite(reportedValue) || reportedValue <= 0) {
        window.alert(
          "Please enter an estimated wastage value before submitting.",
        );
        return;
      }
      ...
      if (reportedValue < minimumAccepted) {
        window.alert(
          `This form cannot be accepted. Entered estimate (${reportedValue} ${baseline.unit}) is below 50% of baseline (${minimumAccepted} ${baseline.unit}).`,
        );
        return;
      }
      ...
      async function _submitReport(reportObj) {
        ...
        document
          .querySelector('[data-priority="Low"]')
          .classList.add("selected-low");
      ...
      if (duplicate) {
        ...
        const daysAgo = Math.floor(
          (Date.now() - new Date(duplicate.timestamp).getTime()) /
            (24 * 60 * 60 * 1000),
        );
        ...
        document.getElementById("dup-submit").onclick = async () => {
          overlay.remove();
  ...
  const STATUS_MAP = {
    Reported: {
      label: "Reported",
      cssClass: "status-reported",
      step: 2,
      hint: "Your report was submitted and sensor data attached. Awaiting officer review.",
    },
    Validated: {
      label: "Validated",
      cssClass: "status-validated",
      step: 3,
      hint: "The Sustainability Officer has confirmed this as a genuine issue.",
    },
    "Forwarded to Finance": {
      label: "Under Finance Review",
      cssClass: "status-finance",
      step: 4,
      hint: "Forwarded to the Finance Analyst for cost-impact assessment.",
    },
    "Cost Impact Added": {
      label: "Under Finance Review",
      cssClass: "status-finance",
      step: 4,
      hint: "Finance is finalizing cost-impact figures for this report.",
    },
    "Returned to SO": {
      label: "Cost Assessed",
      cssClass: "status-cost-assessed",
      step: 4,
      hint: "Finance has added cost data. The officer is preparing the final decision.",
    },
    Finalized: {
      label: "Resolved",
      cssClass: "status-finalized",
      step: 5,
      hint: "The report has been fully resolved and archived.",
    },
    "Target Set": {
      label: "Target Set",
      cssClass: "status-target-set",
      step: 5,
      hint: "A new sustainability target was set based on your report.",
    },
    Delivered: {
      label: "Report Delivered",
      cssClass: "status-finalized",
      step: 6,
      hint: "Detailed findings report has been delivered. Check your wastage logs for the full breakdown.",
    },
    Dismissed: {
      label: "Dismissed",
      cssClass: "status-dismissed",
      step: 3,
      hint: "The officer dismissed this report as not actionable.",
    },
  };
  ...
  const TIMELINE_STEPS = [
    {
      num: 1,
      label: "Observation Logging",
      desc: "You observe and document exactly where waste is happening.",
    },
    {
      num: 2,
      label: "System Cross-Check",
      desc: "We correlate your report with real-time IoT campus sensor data.",
    },
    {
      num: 3,
      label: "Officer Audit",
      desc: "Our Sustainability Officer validates and compiles the footprint report.",
    },
    {
      num: 4,
      label: "Finance Review",
      desc: "Analysts assign ROI impacts to justify process change.",
    },
    {
      num: 5,
      label: "Target Adjusted",
      desc: "Policy and process changes are finalized to prevent future waste.",
    },
    {
      num: 6,
      label: "Report Delivered",
      desc: "Detailed findings report sent back to you. Check My Wastage Logs.",
    },
  ];
        ...
        if (Array.isArray(res)) {
          reports = res.map((r, i) => {
            let offset = r.wastage_report_id
              ? (r.wastage_report_id.charCodeAt(5) || 0) * 3600000
              : i * 86400000;
            ...
            const mapped = {
              ...
              status: r.status
                ...
                : "Reported",
              reporterEmail:
                r.reporter_id === user.user_id ? user.email : "other", // proxy
              reporterName:
                r.reporter_id === user.user_id ? user.name : "Unknown",
  ...
  async function renderTickets() {
    ...
    container.innerHTML = myReports
      .map((t) => {
        ...
        let locationText = "";
        if (t.type === "Energy") {
          summaryText = t.specificData.observation;
          locationText = t.specificData.building;
        } else if (t.type === "Water") {
          summaryText = t.specificData.nature;
          locationText = t.specificData.location;
        } else if (t.type === "Emissions") {
          summaryText = t.specificData.observation;
          locationText = t.specificData.location;
        } else if (t.type === "Food") {
          summaryText =
            t.specificData.typeOfWastage +
            " (" +
            t.specificData.estimatedAmount +
            "kg)";
          locationText = t.specificData.cafeteria;
        }
        ...
        if (isResolved) {
          ...
          let narrativeParts = [];
          narrativeParts.push(
            `This ${t.type.toLowerCase()} wastage report was reviewed and validated by the Sustainability Officer, confirming it as a substantiated concern requiring remediation.`,
          );
          ...
          if (ci) {
            ...
            const roiVal =
              ci.roi != null
                ? ci.roi
                : ci.remediationCost > 0
                  ? ((ci.projectedSavings - ci.remediationCost) /
                      ci.remediationCost) *
                    100
                  : null;
            const paybackVal =
              ci.paybackPeriod != null
                ? ci.paybackPeriod
                : ci.projectedSavings > 0
                  ? ci.remediationCost / ci.projectedSavings
                  : null;
            if (roiVal !== null && paybackVal !== null) {
              narrativeParts.push(
                `The projected return on investment is <strong style="color:#059669;">${roiVal.toFixed(1)}%</strong> with an estimated payback period of <strong style="color:#1e40af;">${paybackVal.toFixed(1)} years</strong>.`,
              );
            }
          }
          ...
          if (mt) {
            narrativeParts.push(
              `In response, the Sustainability Officer has established a revised ${t.type.toLowerCase()} reduction target of <strong style="color:#1e40af;">${mt.value} ${mt.unit}</strong>, effective until <strong>${new Date(mt.deadline).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}</strong>.`,
            );
          }

          if (t.status === "Delivered") {
            narrativeParts.push(
              "This report has been formally closed. All findings, financial assessments, and corrective actions have been documented and delivered for your records.",
            );
          }
          ...
          if (sd.baselineValue && sd.readingValue) {
            ...
            const maxVal =
              Math.max(sd.readingValue, sd.baselineValue, targetVal) * 1.1;
        ...
        const selectedBorder = isSelected
          ? "border-color:#059669;box-shadow:0 0 0 2px rgba(5,150,105,0.15);"
          : "";
  ...
  window._addComment = function (reportId, role) {
    ...
    if (!raw) return;
    const data = JSON.parse(raw);
    const report = (data.workflow.wastageReports || []).find(
      (r) => r.id === reportId,
    );
    if (!report) return;
  ...
  window._selectReport = async function (reportId) {
    ...
    if (report.type === "Energy") {
      const bld = document.getElementById("energyBuilding");
      if (bld) bld.value = sp.building || "";
      const area = document.getElementById("energyArea");
      if (area) area.value = sp.area || "";
      const obs = document.getElementById("energyObservation");
      if (obs) obs.value = sp.observation || "";
      const est = document.getElementById("energyEstimatedValue");
      if (est) est.value = sp.reportedValue || "";
    } else if (report.type === "Water") {
      const loc = document.getElementById("waterLocation");
      if (loc) loc.value = sp.location || "";
      const nat = document.getElementById("waterNature");
      if (nat) nat.value = sp.nature || "";
      const est = document.getElementById("waterEstimatedValue");
      if (est) est.value = sp.reportedValue || "";
    } else if (report.type === "Emissions") {
      const src = document.getElementById("emissionsSource");
      if (src) src.value = sp.source || "";
      const loc = document.getElementById("emissionsLocation");
      if (loc) loc.value = sp.location || "";
      const obs = document.getElementById("emissionsObservation");
      if (obs) obs.value = sp.observation || "";
      const est = document.getElementById("emissionsEstimatedValue");
      if (est) est.value = sp.reportedValue || "";
    } else if (report.type === "Food") {
      const caf = document.getElementById("foodCafeteria");
      if (caf) caf.value = sp.cafeteria || "";
      const typ = document.getElementById("foodType");
      if (typ) typ.value = sp.typeOfWastage || "";
      const amt = document.getElementById("foodAmount");
      if (amt) amt.value = sp.estimatedAmount || "";
    }
    ...
    if (headerDesc)
      headerDesc.textContent = `Status: ${STATUS_MAP[report.status]?.label || report.status}`;
    ...
    if (!newBtn) {
      ...
      newBtn.style.cssText =
        "margin-top:12px;text-align:center;padding:10px 0;cursor:pointer;color:#059669;font-weight:600;font-size:14px;transition:opacity .2s;";
  ...
  window._clearSelection = function () {
    ...
    if (headerDesc)
      headerDesc.textContent =
        "Our Sustainability Officers will directly analyze and resolve this issue.";
