(function () {
  ...
  function validateEmail(isSubmit) {
    ...
    if (val.length > EMAIL_MAX) {
      showError(
        emailError,
        "Email must not exceed " + EMAIL_MAX + " characters.",
        emailInput,
      );
      return false;
    }
    ...
    if (
      !isLetter(localPart[localPart.length - 1]) &&
      !isDigit(localPart[localPart.length - 1])
    ) {
      showError(emailError, "Please enter a valid email address.", emailInput);
      return false;
    }
    ...
    for (var d = 0; d < domainLabels.length; d++) {
      ...
      if (
        !isLetter(label[label.length - 1]) &&
        !isDigit(label[label.length - 1])
      ) {
        showError(
          emailError,
          "Please enter a valid email address.",
    ...
    if (tld.length < 2 || !isAllLetters(tld)) {
      showError(
        emailError,
        "Email must have a valid top-level domain (e.g. .com, .org).",
        emailInput,
      );
      return false;
    }
  ...
  function validatePassword(isSubmit) {
    ...
    if (!hasUpper || !hasLower || !hasNum || !hasSpecial) {
      showError(
        passwordError,
        "Needs: 1 uppercase, 1 lowercase, 1 number, and 1 special character.",
        passwordInput,
      );
      return false;
    }
  ...
  function validateConfirmPassword(isSubmit) {
    ...
    if (val.length === 0) {
      if (isSubmit) {
        ...
        resetField(confirmPwInput, confirmPwError);
      }
      return false;
    }
  ...
  function validateRole(isSubmit) {
    if (!roleSelect.value) {
      if (isSubmit) {
        showError(roleError, "Please select a role.", roleSelect);
      } else {
        resetField(roleSelect, roleError);
      }
      return false;
    }
  ...
  nameInput.addEventListener("input", function () {
    if (this.value.length > NAME_MAX)
      this.value = this.value.slice(0, NAME_MAX);
  });
  emailInput.addEventListener("input", function () {
    if (this.value.length > EMAIL_MAX)
      this.value = this.value.slice(0, EMAIL_MAX);
  });
  ...
  form.addEventListener("submit", async function (e) {
    ...
    var emailTaken =
      typeof isEmailTaken === "function"
        ? isEmailTaken(emailInput.value.trim())
        : false;
    var phoneTaken =
      typeof isPhoneTaken === "function"
        ? isPhoneTaken(phoneInput.value.trim())
        : false;
    ...
    if (emailTaken || phoneTaken) {
      ...
      if (emailTaken && phoneTaken) {
        popupMsg.textContent =
          "Both this email and phone number are already registered.";
      } else if (emailTaken) {
