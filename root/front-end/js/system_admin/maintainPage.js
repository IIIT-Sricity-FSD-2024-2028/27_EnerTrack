/**
 * maintainPage.js
 * Entry-point for system_admin_maintain.html.
...
function wireAddButtons() {
  if (!roleAllowed(["system_admin", "superuser"])) return;
...
window.viewSystemLogs = () => {
  if (!roleAllowed(["system_admin", "superuser"])) {
    showToast("Access denied: system_admin privileges required to view logs.", "error");
    return;
  }
  openModal({
    title: "System Logs",
...
window.restartServices = () => {
  if (!roleAllowed(["system_admin", "superuser"])) {
    showToast("Access denied: system_admin privileges required.", "error");
    return;
  }
  openModal({
