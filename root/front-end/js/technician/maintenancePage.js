...
document.addEventListener('click', async e => {
    const fault = selectedFaultId ? _faults.find(f => (f.fault_id || f.id) === selectedFaultId) : null;
    ...
    if (e.target.id === 'btnEditPrelim') {
        document.getElementById('prelimViewMode').style.display = 'none';
        document.getElementById('prelimEditMode').style.display = 'block';
        document.getElementById('prelimTextarea').value = fault?.prelim_notes || fault?.prelimNotes || '';
    }
    ...
    if (e.target.id === 'btnEditQuickfix') {
        document.getElementById('quickfixViewMode').style.display = 'none';
        document.getElementById('quickfixEditMode').style.display = 'block';
        document.getElementById('quickfixTextarea').value = fault?.quickfix_notes || fault?.quickfixNotes || '';
    }
        ...
        openModal({
            ...
            onConfirm: async () => {
                ...
                if (fault?.alert_id && window.api) {
                    window.api.patch(`/alerts/${fault.alert_id}`, { status: 'resolved' }).catch(() => {});
                }
...
function getWOModalHTML(fault, type) {
    const techs = _users.filter(u => u.role === 'Technician' || u.role === 'Technician Administrator');
    ...
    const finalTechOptions = techOptions || `<option value="Unassigned">Unassigned</option>`;
