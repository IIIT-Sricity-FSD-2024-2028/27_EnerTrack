...
async function loadAlerts() {
  ...
  window.resolveAlertFn = async function (id) {
    try {
      if (window.api)
        await window.api.patch(`/alerts/${id}`, { status: "resolved" });
...
function renderHistoryLog() {
  ...
  historyAlerts.sort(
    (a, b) => (sevScore[b.severity] || 0) - (sevScore[a.severity] || 0),
  );

  tbody.innerHTML = historyAlerts
    .map((a) => {
      const id = a.alert_id || a.id;
...
function selectAlert(id) {
  ...
  document
    .querySelectorAll("#alertQueueBody tr")
    .forEach((r) => r.classList.remove("selected"));
  document
    .querySelector(`#alertQueueBody tr[data-alert-id="${id}"]`)
  ...
  if (btnEmergency)
    btnEmergency.onclick = () =>
      acknowledgeAlert(id, "Emergency Response Initiated");
  if (btnSchedule)
      ...
      openModal({
        ...
        onConfirm: () => {
          ...
          if (next) selectAlert(next.alert_id || next.id);
            ...
            setEl(
              "detailDescription",
              "All alerts have been resolved or acknowledged.",
            );
          }
        },
      });
...
async function acknowledgeAlert(id, actionTaken) {
  try {
    if (window.api)
      await window.api.patch(`/alerts/${id}`, { status: "acknowledged" });
