...
export function renderCostTable(filter = {}) {
  ...
  if (filter.scope && filter.scope !== "all")
    records = records.filter((r) => r.scope === filter.scope);
...
export function openAddCostModal() {
  ...
  const deptOptions = FinanceDB.departments
    .map((d) => `<option value="${d.id}|dept|${d.name}">${d.name}</option>`)
    .join("");
  const bldgOptions = FinanceDB.buildings
    .map((b) => `<option value="${b.id}|building|${b.name}">${b.name}</option>`)
    .join("");
...
export function _toggleScopeSelect() {
  ...
  document.getElementById("ac-scope-dept").style.display =
    type === "department" ? "" : "none";
  document.getElementById("ac-scope-bldg").style.display =
    type === "building" ? "" : "none";
}
...
function _submitAddCost() {
  ...
  if (scopeType === "department") {
    ...
    if (!scopeValue) {
      showFieldError(deptRef, "Select a department.");
      valid = false;
    }
  } else if (scopeType === "building") {
    scopeValue = bldgRef?.value ?? "";
    if (!scopeValue) {
      showFieldError(bldgRef, "Select a building.");
      valid = false;
    }
  }
  ...
  function _finishAdd(rec) {
    ...
    logActivity(
      "energy",
      `Cost record added for ${scopeLabel}`,
      `Period: ${periodVal}, Total: ${formatCurrency(total)}`,
    );
    renderCostTable();
    updateCostSummary();
...
function _submitEditCost(id) {
  ...
  function _finishEdit() {
    ...
    logActivity(
      "energy",
      `Cost record updated for ${FinanceDB.energyCosts[idx].scopeLabel}`,
...
export function deleteCostRecord(id) {
  ...
  openModal({
    ...
    onConfirm: () => {
      if (window.api) {
        window.api
          .delete(`/energy-costs/${id}`)
          .then(() => {
            universalDB.data.finance.energyCosts = FinanceDB.energyCosts.filter(
              (r) => r.id !== id,
            );
            _finishDelete();
          })
      ...
      function _finishDelete() {
        ...
        logActivity(
          "energy",
          `Cost record deleted for ${rec.scopeLabel}`,
          `Period: ${rec.period}`,
        );
        renderCostTable();
        updateCostSummary();
...
function _setAnalyticsPending() {
  ...
  if (bars)
    bars.innerHTML = `<div style="width:100%;text-align:center;color:#9ca3af;padding-top:20px;font-style:italic">Analytics pending. Click "Retrieve & Calculate" to begin simulation.</div>`;

  // Reset metrics
  document
    .querySelectorAll(".metric strong")
    .forEach((s) => (s.textContent = "—"));
  document
    .querySelectorAll(".budget-card strong")
    .forEach((s) => (s.textContent = "—"));
}
...
export function renderCostBreakdown(viewMode) {
  ...
  if (viewMode === "energy") {
    data = [
      {
        label: "Electricity",
        value: all.reduce((s, r) => s + r.electricity, 0),
      },
      { label: "Gas", value: all.reduce((s, r) => s + r.gas, 0) },
      { label: "Water", value: all.reduce((s, r) => s + r.water, 0) },
      {
        label: "Wastewater",
        value: all.reduce((s, r) => s + (r.wastewater || 0), 0),
      },
      {
        label: "Demand",
        value: all.reduce((s, r) => s + r.demand, 0),
        isDemand: true,
      },
    ];
