...
async function renderFinanceWorkflow() {
  ...
  try {
    if (window.api) {
      ...
      if (Array.isArray(woRes))
        internalWOs = woRes.filter((w) => w.status === "approval");
    }
  } catch (e) {
  ...
  const pendingEstimates = srs.filter(
    (sr) => sr.status === "Awaiting Estimate Approval",
  );
  const validatedJobs = srs.filter(
    (sr) => sr.status === "Validated (Awaiting Payment)",
  );

  if (
    pendingEstimates.length === 0 &&
    validatedJobs.length === 0 &&
    internalWOs.length === 0
  ) {
    container.innerHTML =
      '<div style="padding:20px; color:#8a95a2; font-size:14px; text-align:center;">No pending maintenance or payment authorizations.</div>';
    return;
  }
...
window.rejectWOEstimate = async function (id) {
  try {
    ...
    if (res && !res.error) {
      showToast(
        "Internal Work Order estimate rejected. Tech notified.",
        "warning",
      );
      await renderFinanceWorkflow();
    } else {
...
function renderSummaryCards() {
  ...
  if (changeLabels[0])
    changeLabels[0].textContent = `Latest period (${latestPeriod})`;
    ...
    if (window.api) {
      ...
      if (Array.isArray(res)) {
        return res.map((r) => ({
          ...
          status: r.status,
          reporterEmail:
            r.reporter_id === currentUser.user_id ? currentUser.email : "other",
          reporterName:
            r.reporter_id === currentUser.user_id
              ? currentUser.name
              : "Unknown",
...
async function renderWastageReviewQueue() {
  ...
  const actionable = allReports.filter(
    (r) =>
  ...
  if (countBadge) {
    countBadge.textContent =
      actionable.length > 0 ? `${actionable.length} pending` : "";
  }
...
function _buildReportCard(r) {
  ...
  if (r.status === "Forwarded to Finance") {
    ...
    statusBg = "#d1fae5";
    statusColor = "#065f46";
    statusLabel = "Cost Impact Added";
  }

  // Validation timestamp (from SO)
  const validatedAt = r.validatedAt
    ? new Date(r.validatedAt).toLocaleDateString("en-GB", {
        day: "numeric",
        month: "short",
        year: "numeric",
        hour: "2-digit",
  ...
  if (r.costImpact) {
    ...
    const payback =
      ci.projectedSavings > 0 ? ci.remediationCost / ci.projectedSavings : null;
    const roi =
      ci.remediationCost > 0
        ? ((ci.projectedSavings - ci.remediationCost) / ci.remediationCost) *
          100
        : null;
    costImpactHTML = `
    ...
    onConfirm: async () => {
      ...
      const costImpact = {
        ...
        roi:
          parseFloat(remVal) > 0
            ? +(
                ((parseFloat(savVal) - parseFloat(remVal)) /
                  parseFloat(remVal)) *
                100
              ).toFixed(1)
            : 0,
        paybackPeriod:
          parseFloat(savVal) > 0
            ? +(parseFloat(remVal) / parseFloat(savVal)).toFixed(2)
            : 0,
        notes: notesVal || "",
      ...
      await _updateWastageReport(reportId, {
        status: "Cost Impact Added",
        details: { costImpact },
      });

      showToast(
        "Cost impact saved. Review and return to SO when ready.",
        "success",
      );
...
window._sendBackToSO = async function (reportId) {
  ...
  showToast(
    "Report sent back to Sustainability Officer with cost figures.",
    "success",
  );
