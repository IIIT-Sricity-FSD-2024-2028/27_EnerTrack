...
function PIPELINE_KEY() {
  return getUserKey("et_sust_pipeline_state");
}
function SYNC_BTN_KEY() {
  return getUserKey("et_sust_sync_btns");
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  try {
    ...
    console.error(
      "EnerTrack Monitoring System: Critical initialization error:",
      err,
    );
  }
});
...
async function initMonitoring() {
  ...
  try {
    if (window.api) {
      ...
      const metrics = Array.isArray(metricsRes)
        ? metricsRes
        : metricsRes?.data || [];
      if (metrics.length > 0) {
        // sort to get latest
  ...
  document.querySelectorAll(".btn-refresh").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      setTimeout(() => {
        ...
        saveSyncButtonState();
        showToast(
          `${metric.charAt(0).toUpperCase() + metric.slice(1)} metric refreshed.`,
          "success",
        );
      }, 1000);
    });
...
function restorePipelineState() {
  ...
  if (state === "completed") {
    ...
    document
      .querySelectorAll(".btn-refresh")
      .forEach((btn) => setButtonUpdated(btn));
    saveSyncButtonState();
  }
}
...
function runPipelineSequence() {
  ...
  fill.style.width = "0%";
  steps.forEach((s) =>
    s.querySelector(".step-icon").classList.remove("active"),
  );
  ...
  const progressInterval = setInterval(() => {
    ...
    const stepEl = document.querySelector(
      `#monitoringPipeline .step[data-step="${currentStep}"]`,
    );
    if (stepEl) {
  ...
  function finishPipeline() {
    ...
    SustDB.addHighlight(
      "Data Synced",
      "Monitoring pipeline completed automatically",
      "blue",
    );
    ...
    document
      .querySelectorAll(".btn-refresh")
      .forEach((b) => setButtonUpdated(b));
    saveSyncButtonState();

    showToast(
      "Monitoring cycle completed. Dashboard updated with live feeds.",
      "success",
    );
