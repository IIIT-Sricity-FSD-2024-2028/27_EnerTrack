...
export class InitiativesService {
  ...
  create(createDto: CreateInitiativeDto) {
    ...
    const newRecord = { initiative_id: generatedId, ...createDto, organization_id: currentOrgId() ?? createDto.organization_id ?? null };
    this.database.initiatives.push(newRecord as any);
    return newRecord;
  }

  findAll() {
    return scopeToTenant(this.database.initiatives);
  }

  findOne(id: string) {
    const record = assertTenantOwns(this.database.initiatives.find(
  ...
  put(id: string, putDto: PutInitiativeDto) {
    ...
    if (index === -1 || !assertTenantOwns(this.database.initiatives[index]))
      throw new NotFoundException(`Initiative with ID ${id} not found`);
    this.database.initiatives[index] = {
      initiative_id: id,
      ...putDto,
      organization_id: this.database.initiatives[index].organization_id,
    } as any;
