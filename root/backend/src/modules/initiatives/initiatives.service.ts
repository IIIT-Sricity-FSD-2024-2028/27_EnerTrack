...
export class InitiativesService {
  ...
  create(createDto: CreateInitiativeDto) {
    if (createDto.created_by_id) {
      ...
      if (!exists) throw new NotFoundException(`Target users with id '${createDto.created_by_id}' not found`);
    }
  ...
  findOne(id: string) {
    ...
    if (!record) throw new NotFoundException(`Initiative with ID ${id} not found`);
    return record;
  }

  update(id: string, updateDto: UpdateInitiativeDto) {
    const index = this.database.initiatives.findIndex(item => item.initiative_id === id);
    if (index === -1) throw new NotFoundException(`Initiative with ID ${id} not found`);
