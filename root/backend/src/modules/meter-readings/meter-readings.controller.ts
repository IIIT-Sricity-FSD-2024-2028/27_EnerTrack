...
export class MeterReadingsController {
  ...
  @Post()
  @ApiOperation({ summary: "Create Meter Reading", description: "Records a new meter reading from an IoT sensor. The Organization Admin and Technician can submit readings by providing meter_id, value, and unit. Send a POST request with a CreateMeterReadingDto JSON body." })
  @ApiResponse({ status: 201, description: "Meter reading created successfully." })
  @ApiResponse({ status: 403, description: "Forbidden – caller role is not Organization Admin or Technician." })
...
@ApiOperation({ summary: "Bulk Upload Meter Readings", description: "Uploads a CSV file of meter readings for sites without automatic sensors. The Organization Admin and Technician can submit a batch of readings via file. Send a multipart/form-data POST request with the CSV file under the 'file' field." })
...
uploadSpreadsheet(@UploadedFile() file: Express.Multer.File) {
  ...
  @Get()
  @ApiOperation({ summary: "List All Meter Readings", description: "Retrieves all meter reading records across the system. The Organization Admin, Technician, Financial Analyst, and Sustainability Officer can view the full history of sensor readings. Send a GET request with no additional parameters." })
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get Meter Reading by ID", description: "Retrieves a single meter reading record by its UUID including meter_id, value, unit, and timestamp. The Organization Admin, Technician, Financial Analyst, and Sustainability Officer can look up a specific reading. Pass the reading_id as a URL path parameter." })
  ...
  @Put(":id")
  @ApiOperation({ summary: "Replace Meter Reading", description: "Completely replaces an existing meter reading record with new data. Only the Organization Admin can perform full reading replacements. Send a PUT request with the reading_id in the URL and a complete PutMeterReadingDto JSON body." })
  ...
  @Patch(":id")
  @ApiOperation({ summary: "Update Meter Reading", description: "Partially updates specific fields of an existing meter reading (e.g., correcting a value or unit). Only the Organization Admin can modify readings. Send a PATCH request with the reading_id in the URL and an UpdateMeterReadingDto JSON body containing only the fields to change." })
  ...
  @Delete(":id")
  @ApiOperation({ summary: "Delete Meter Reading", description: "Permanently removes a meter reading record from the system. Only the Organization Admin can delete readings. Pass the reading_id as a URL path parameter." })
