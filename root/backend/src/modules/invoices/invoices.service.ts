...
export class InvoicesService {
  ...
  create(createDto: CreateInvoiceDto) {
    if (createDto.invoice_number) {
      ...
      if (exists) throw new ConflictException(`Duplicate invoice_number '${createDto.invoice_number}'`);
    }
    if (createDto.department_id) {
      const exists = this.database.departments.find(x => x.department_id === createDto.department_id);
      if (!exists) throw new NotFoundException(`Target departments with id '${createDto.department_id}' not found`);
    }
    if (createDto.approved_by_id) {
      const exists = this.database.users.find(x => x.user_id === createDto.approved_by_id);
      if (!exists) throw new NotFoundException(`Target users with id '${createDto.approved_by_id}' not found`);
    }
  ...
  update(id: string, updateDto: UpdateInvoiceDto) {
    ...
    if (index === -1) throw new NotFoundException(`Invoice with ID ${id} not found`);
