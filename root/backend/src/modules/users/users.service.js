...
var UsersService = function () {
    ...
    var UsersService = _classThis = /** @class */ (function () {
        function UsersService_1(database) {
            this.database = database;
        }
        UsersService_1.prototype.create = function (createDto) {
            var id = Date.now().toString() + Math.random().toString(36).substring(7);
            var newRecord = __assign({ user_id: id }, createDto);
        ...
        UsersService_1.prototype.findOne = function (id) {
            var record = this.database.users.find(function (item) { return item.user_id === id; });
            if (!record)
                throw new common_1.NotFoundException("User with ID ".concat(id, " not found"));
            return record;
        };
        UsersService_1.prototype.update = function (id, updateDto) {
            var index = this.database.users.findIndex(function (item) { return item.user_id === id; });
            if (index === -1)
                throw new common_1.NotFoundException("User with ID ".concat(id, " not found"));
            this.database.users[index] = __assign(__assign({}, this.database.users[index]), updateDto);
        ...
        UsersService_1.prototype.remove = function (id) {
            ...
            if (index === -1)
                throw new common_1.NotFoundException("User with ID ".concat(id, " not found"));
            var removed = this.database.users.splice(index, 1);
            return removed[0];
        };
