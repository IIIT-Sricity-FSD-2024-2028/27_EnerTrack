...
export class FaultsService {
  ...
  create(createDto: CreateFaultDto) {
    if (createDto.alert_id) {
      ...
      if (!exists) throw new NotFoundException(`Target alerts with id '${createDto.alert_id}' not found`);
    }
  ...
  update(id: string, updateDto: UpdateFaultDto) {
    ...
    if (index === -1) throw new NotFoundException(`Fault with ID ${id} not found`);
