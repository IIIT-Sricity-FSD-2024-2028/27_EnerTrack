...
export class DepartmentsController {
  ...
  @Post()
  @ApiOperation({ summary: "Create Department", description: "Creates a new department within a building. Only the Organization Admin can add departments by providing building_id, name, and optional budget. Send a POST request with a CreateDepartmentDto JSON body." })
  ...
  @Put(":id")
  @ApiOperation({ summary: "Replace Department", description: "Completely replaces an existing department record with new data. Only the Organization Admin can perform full department replacements. Send a PUT request with the department_id in the URL and a complete PutDepartmentDto JSON body." })
  ...
  @Patch(":id")
  @ApiOperation({ summary: "Update Department", description: "Partially updates specific fields of an existing department (e.g., name or budget). Only the Organization Admin can modify department details. Send a PATCH request with the department_id in the URL and an UpdateDepartmentDto JSON body containing only the fields to change." })
  ...
  @Delete(":id")
  @ApiOperation({ summary: "Delete Department", description: "Permanently removes a department record from the system. Only the Organization Admin can delete departments. Pass the department_id as a URL path parameter." })
  ...
  @Get(":id/invoices")
  @ApiOperation({ summary: "Get Invoices for Department", description: "Retrieves all invoice records linked to a specific department (1:N relationship). The Organization Admin and Financial Analyst can view invoices for a department. Pass the department_id as a URL path parameter." })
  @ApiResponse({ status: 200, description: "Array of invoice records for the department returned." })
  @ApiResponse({ status: 403, description: "Forbidden (RBAC)" })
  @ApiHeader({ name: "x-role", description: "User role for RBAC. Enum: Organization Admin | Financial Analyst | Technician | Sustainability Officer | Campus Visitor", required: false })
  @Roles("Organization Admin", "Financial Analyst")
