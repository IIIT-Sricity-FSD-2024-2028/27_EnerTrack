...
export class WastageReportsService {
  ...
  create(createDto: CreateWastageReportDto) {
    ...
    if (createDto.sensor_reading_id) {
      ...
      if (!exists) throw new NotFoundException(`Target meterReadings with id '${createDto.sensor_reading_id}' not found`);
    }
  ...
  findOne(id: string) {
    ...
    if (!record) throw new NotFoundException(`WastageReport with ID ${id} not found`);
    return record;
  }
  ...
  update(id: string, updateDto: UpdateWastageReportDto) {
    ...
    if (index === -1) throw new NotFoundException(`WastageReport with ID ${id} not found`);
